    ...
    <nav className="relative z-50 flex flex-row justify-between items-center px-6 md:px-8 py-6 max-w-7xl mx-auto w-full">
      ...
      <div className="flex items-center gap-4">
        {user ? (
            ...
            <div className="absolute top-full right-0 mt-2 w-48 rounded-xl border border-[var(--border)] bg-black/80 backdrop-blur-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 flex flex-col overflow-hidden">
              <div className="px-4 py-3 border-b border-[var(--border)]">
                ...
                <p className="text-xs text-zinc-400 mt-1">
                  {profile?.isTester ? 0 : (profile?.xp || 0)} XP
                </p>
              </div>
          ...
          <>
            <button onClick={() => setShowSignInModal(true)} className="hidden md:block text-sm font-medium text-[var(--muted-fg)] hover:text-[var(--fg)] transition-colors cursor-pointer">
              Sign In
            </button>
            <button onClick={() => setShowSignInModal(true)} className="liquid-glass rounded-full px-4 md:px-6 py-2 md:py-2.5 text-xs md:text-sm text-[var(--fg)] hover:scale-[1.03] transition-all cursor-pointer hover:bg-gradient-to-r hover:from-blue-600 hover:to-red-600 hover:text-white">
              Join
            </button>
          </>
        )}
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-2xl md:hidden flex flex-col items-center justify-center gap-8 animate-in fade-in duration-300">
          ...
          {user ? (
            ...
            <div className="flex flex-col items-center gap-4 mt-4 pt-8 border-t border-[var(--border)] w-full max-w-[200px]">
              <button onClick={() => { setShowSignInModal(true); setIsMenuOpen(false); }} className="text-xl font-serif text-[var(--fg)] cursor-pointer">Sign In</button>
              <button onClick={() => { setShowSignInModal(true); setIsMenuOpen(false); }} className="liquid-glass rounded-full px-10 py-4 text-lg text-[var(--fg)] mt-2 cursor-pointer">
                Join
              </button>
            </div>
          )}
